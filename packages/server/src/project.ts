...
export class ProjectManager extends EventEmitter {
  ...
  private async persistConfig(): Promise<void> {
    ...
    if (this.organizationSource === 'manifest') {
      ...
      const out = {
        ...
        strictTypes: this.config.strictTypes,
        inferTypes: this.config.inferTypes,
        ...(this.config.palettes && this.config.palettes.length > 0
  ...
  getPalettes(): PaletteRecipe[] {
    return this.config.palettes ?? [];
  }

  async savePalette(recipe: PaletteRecipe): Promise<PaletteRecipe[]> {
    const key = paletteKey(recipe.collection, recipe.groupPath);
    const rest = (this.config.palettes ?? []).filter(
      (p) => paletteKey(p.collection, p.groupPath) !== key,
    );
    this.config = { ...this.config, palettes: [...rest, recipe] };
    await this.persistConfig();
    return this.getPalettes();
  }

  async deletePalette(collection: string, groupPath: string[]): Promise<PaletteRecipe[]> {
    const key = paletteKey(collection, groupPath);
    const next = (this.config.palettes ?? []).filter(
      (p) => paletteKey(p.collection, p.groupPath) !== key,
    );
    this.config = { ...this.config, palettes: next.length > 0 ? next : undefined };
    await this.persistConfig();
    return this.getPalettes();
  }
